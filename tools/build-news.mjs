...
function extractJsonArrayFromResponses(resp) {
  ...
  if (!content) throw new Error("Réponse OpenAI vide ou malformée.");
  try {
    return JSON.parse(content);
  } catch {
    throw new Error("Impossible de parser le JSON structuré de la réponse OpenAI.");
  }
}
...
async function analyzeWithOpenAI(items, W) {
  ...
  const systemMsg = "Tu es un assistant de veille pour l'éducation numérique/IA.\n" +
    "Analyse les articles fournis et retourne un tableau JSON d'objets respectant le schéma demandé.\n" +
    buildPromptWeights(W) +
    "\nPriorise: (i) articles/journaux/conférences, (ii) politiques/régulation/standards, (iii) communiqués d'institutions.";

  const userMsg = "Articles à analyser :\n" + items.map((it, idx) => `[${idx}] ${it.title}\nURL: ${it.url}\nSnippet: ${it.snippet || ""}`).join("\n---\n");
  ...
  const data = extractJsonArrayFromResponses(resp);
  return data.items;
}
