...
// Si oui, préfixe la question avec un extrait texte du PDF: /CV_Raphael_Tuor.pdf
...
(function () {
  ...
  async function loadCv() {
    ...
    try {
      ...
      const CV_URL = "/assets/cv/cv-fr.pdf";
      const doc = await window.pdfjsLib.getDocument(CV_URL).promise;
      let out = "";
  ...
  function looksLikeCvQuestion(q) {
    ...
    return /\b(cv|curriculum|résumé|resume|lebenslauf|parcours|expériences?|experience|skills?|compétences|dipl[oô]me|formation|linkedin|nicolas|raphael|tuor)\b/.test(
      s
    );
  }
